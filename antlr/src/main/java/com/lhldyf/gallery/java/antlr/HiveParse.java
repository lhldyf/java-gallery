...
public class HiveParse {
        ...
        if (ast.getToken() != null) {
            switch (ast.getToken().getType()) {
                ...
                case HiveParser.TOK_TAB:// outputTable
                    String tableTab = ParseUtils.getUnescapedName((ASTNode) ast.getChild(0));
                    if (oper == Oper.SELECT) {
                ...
                case HiveParser.TOK_TABREF:// inputTable
                    ...
                    String tableName = (tabTree.getChildCount() == 1) ?
                            ParseUtils.getUnescapedName((ASTNode) tabTree.getChild(0)) :
                            ParseUtils.getUnescapedName((ASTNode) tabTree.getChild(0)) + "." + tabTree.getChild(1);
                    if (oper == Oper.SELECT) {
                        ...
                        if (ast.getChildCount() == 2) {
                            if (ast.getChild(0).getType() == HiveParser.TOK_TABLE_OR_COL
                                ...
                                if (!tables.contains(alia + "\t" + oper)
                                        && alias.get(alia) == null) {// [b SQL_TYPE_SELECT, a
                                    // SQL_TYPE_SELECT]
    ...
    public static void main(String[] args) throws IOException, ParseException, SemanticException {
        ...
        String sql6 = "FROM (  SQL_TYPE_SELECT p.datekey datekey, p.userid userid, c.clienttype  FROM detail"
                + ".usersequence_client c " + "JOIN fact.orderpayment p ON p.orderid = c.orderid "
                + " JOIN default.user du ON du.userid = p.userid WHERE p.datekey = 20131118 ) base  "
                + "SQL_TYPE_INSERT " + "OVERWRITE TABLE `test`.`customer_kpi` SQL_TYPE_SELECT base.datekey, "
                + "  base.clienttype, count(distinct base.userid) buyer_count GROUP BY base.datekey, base"
                + ".clienttype";
        String sql7 =
                "SQL_TYPE_SELECT id, value FROM (SQL_TYPE_SELECT id, value FROM p1 UNION ALL  SQL_TYPE_SELECT 4 AS "
                        + "id, 5 AS value FROM p1 limit " + "1) u";
        ...
        String sql16 =
                "select statis_date,time_interval,gds_cd,gds_nm,sale_cnt,discount_amt,discount_rate,price,etl_time,"
                        + "pay_amt from o2ostore.tdm_gds_monitor_rt where time_interval = from_unixtime"
                        + "(unix_timestamp(concat(regexp_replace(from_unixtime(unix_timestamp('201506181700', "
                        + "'yyyyMMddHHmm')+ 84600 ,  'yyyy-MM-dd HH:mm'),'-| |:',''),'00'),'yyyyMMddHHmmss'),"
                        + "'yyyy-MM-dd HH:mm:ss')";
        String sql13 = "SQL_TYPE_INSERT OVERWRITE TABLE u_data_new SQL_TYPE_SELECT TRANSFORM (userid, movieid, rating, "
                + "unixtime) USING 'python" + " weekday_mapper.py' AS (userid, movieid, rating, weekday) FROM u_data";
        String sql14 = "SQL_TYPE_SELECT a.* FROM a JOIN b ON (a.id = b.id AND a.department = b.department)";
        ...
        String sql22 =
                "select login.uid from login day_login left outer join (select uid from regusers where dt='20130101')"
                        + " day_regusers on day_login.uid=day_regusers.uid where day_login.dt='20130101' and "
                        + "day_regusers.uid is null";
        String sql23 = "select name from (select * from zpc left outer join def) d";
        String sql24 = "SQL_TYPE_SELECT table_name AS name, database_name, create_time, table_type, owner_name, "
                + "commentstring, "
                + "transactional, table_format, table_location, NULL AS origin_text, NULL AS expanded_text, "
                + "'TABLE' AS object_type FROM system.tables_v WHERE database_name='default' ORDER BY name "
                + "UNION ALL SQL_TYPE_SELECT view_name AS name, database_name, TIMESTAMP('1970-01-01 0:0:0') " + "AS "
                ...
                + "'VIEW' AS object_type FROM system.views_v WHERE database_name='default' ORDER BY name";
        String sql25 = "SQL_TYPE_SELECT * FROM fqtxt";
        String sql26 =
                "SQL_TYPE_SELECT * FROM system.columns_v WHERE database_name='default' AND table_name='fqtxt' ORDER BY "
                        + "column_id";
        String sql27 = "SHOW CREATE TABLE orc_sin_tbl";
        String sql28 = "SQL_TYPE_INSERT INTO TABLE ta VALUES ('sssss','44')";
        String sql29 = "insert into table hs_tmp.hbase_ods_act_client select client_id,id_enddate,contact_id_enddate,"
                + "id_kind,contact_tel,remark,crdtval_kind,client_id,open_business,mail_way,contact_freq,"
                + "id_no,statement_flag,cost_place,bill_template_type,contact_id_begindate,position_str,"
                + "receive_name,cancel_date,prodasset_flag,risk_info,organ_flag,fund_password,client_name,"
                + "open_date,contact_id_no,develop_source,contact_name,open_type,en_contact_type,develop_net,"
                + "sign_date,contact_id_kind,trade_password,branch_no,full_name,sign_flag,client_status,"
                + "confirm_date,contact_mobile,account_data,developer,id_begindate,cardread_flag,cust_id from"
                + " hs_ods.ods_act_client1";
        String sql30 = "grant create on database tdt to role public";
        String sql31 = "revoke all on database spark from user mllib;";
        String sql32 =
                "insert overwrite table risk_event partition(year='2019',dt) select t.ops as order_no,t.id_no , concat"
                        ...
                        + "t1.consumer_no";
        String sql33 = "SQL_TYPE_UPDATE t2 SET name='张' WHERE id=1;";
        String parsesql = sql2;
        HiveParse hp = new HiveParse();
        System.out.println(parsesql);
        ASTNode ast = pd.parse(parsesql, null, conf);
        System.out.println(ast.toStringTree());
        long start = System.currentTimeMillis();
        System.out.println(pd.parse(sql30, null, conf).toStringTree());
        System.out.println(pd.parse(sql30, null, conf).toStringTree());
